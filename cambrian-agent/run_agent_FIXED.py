#!/usr/bin/env python3
"""
FIXED Cambrian Trading Agent using MCP
This correctly configures the MCP server
"""
...
class CambrianMCPAgent:
    ...
    def __init__(self):
        ...
        # Load the MCP config to get the fluora server details
        with open('../config/mcp_config.json') as f:
            mcp_config = json.load(f)
        
        fluora_config = mcp_config['mcpServers']['fluora']
        
        # Create the MCP server configuration
        self.mcp_server_config = McpStdioServerConfig(
            command=fluora_config['command'],
            args=fluora_config['args'],
            env=fluora_config['env']
        )
